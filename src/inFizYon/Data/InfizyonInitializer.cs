...
namespace inFizYon.Data
{
    public static class AcademyInitializer
    {
        ...
        {
            //: System.Data.Entity. DropCreateDatabaseIfModelChanges<InfizyonDbContext>
            ...
            // protected override void Seed(inFizYonDbContext academycontext)
            //{
            ...
            //
            //    academycontext.People.AddOrUpdate(
            //      p => p.FullName,
            ...
            //var hasLabel = new List<Label>
            //{            
            //    new Label{  Labeltxt="00 00 00", Origin = Label.MetaData.MasterFormat, Phrases = new List<Phrase>()}
            //};

            //hasLabel.ForEach(c => academycontext.LabelsSet.AddOrUpdate(c));
            //academycontext.SaveChanges();
            ...
            //    new Phrase{phrsENtxt = "Special report requirements and instructions justifying product content, origin, or other attributes, for projects requiring special sustainability provisions, to meet LEEDTM, requirements, or other sustainable goals.", phrsTRtxt = "LEED ya da benzeri özel sürdürülebilirlik şartları gerektiren projeler için hazırlanması gereken; ürün kapsamı, kökeni ve diğer özelliklerine ait özel rapor ve talimatları içerir.", phrsReliance =100, phrsComp =100},
            ...
            //Phrases.ForEach(p => academycontext.PhraseSet.AddOrUpdate(p));
            //academycontext.SaveChanges();

            ////Insert sample MasterFormat Section
            //var MFSections = new List<inMF>
            //{
            //    new inMF{ SecNr="00 00 00", SectionID = Phrases.Single(s => s.phrsTRtxt == "Test satırı").phrsID}
            //};

            //MFSections.ForEach(m => academycontext.MFSectionSet.AddOrUpdate(m));
            //academycontext.SaveChanges();

            ////Insert default Project
            //var inProjects = new List<inProject>
            //{            
            //    new inProject{ prjCode = "IFC_HGYO", prjPlace = "Istanbul Finance Centre - Ataşehir",  prjID = Phrases.Single(n => n.phrsTRtxt == "İstanbul Finans Merkezi - Ataşehir").phrsID}
            //};

            //inProjects.ForEach(i => academycontext.Projects.AddOrUpdate(i));
            //academycontext.SaveChanges();
            ...
            ////newPackage.ForEach(pa => academycontext.PackageSet.AddOrUpdate(pa));
            ////academycontext.SaveChanges();

            ////Insert default Comment
            //var hasComment = new List<Comment>
            //{            
            //    new Comment{ Commenttxt="Default comment", Commented = Phrases.Single(c => c.phrsENtxt == "Test datum")}
            //};

            //hasComment.ForEach(c => academycontext.hasCommentSet.AddOrUpdate(c));
            //academycontext.SaveChanges();
